...
\NormalTok{p.survey }\OtherTok{\textless{}{-}} \FloatTok{0.025} \CommentTok{\# Default capture probability for survey}
...
\NormalTok{lambda }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n.years}\DecValTok{{-}1}\NormalTok{, mean.lambda, sigma.lambda) }
\CommentTok{\# Lambda is vector of annual population growth rates}
...
For convenience in viewing the results, we skip plotting the jagsfit object and simply add the regression line to the original plot of survey catches (Figure \ref{fig:LinTrendPlot}). We can examine the credible interval for \(b_2\) to determine whether the survey provides evidence of a trend (credible interval not including 0). A more quantitative approach is to estimate the probability of a positive mean growth rate \citep{pregler.etal_2019}. We can use the step() function\index{step function} (which is 1 for an argument \textgreater{} 0; 0 otherwise) to calculate the fraction of retained slope estimates that are greater than 0 (mean for calculated variable p.pos).

\begin{figure}
\includegraphics[width=0.9\linewidth]{bookdown_files/figure-latex/LinTrendPlot-1} \caption{Example of fitted linear model for survey catch versus year.}\label{fig:LinTrendPlot}
\end{figure}
...
\StringTok{model \{}
\StringTok{\# Priors}
\StringTok{logC.est[1] \textasciitilde{} dunif(1, 100)     \# Initial relative abundance}
\StringTok{mean.r.est \textasciitilde{} dnorm(0, 0.001)    \# Mean growth rate}
\StringTok{sigma.proc.est \textasciitilde{} dunif(0, 1)  \# Process error (annual variation in r)}
\StringTok{tau.proc.est \textless{}{-} pow(sigma.proc.est, {-}2)}
\StringTok{sigma.obs.est \textasciitilde{} dunif(0, 100)     \# Observation error}
\StringTok{tau.obs.est \textless{}{-} pow(sigma.obs.est, {-}2)}

\StringTok{\# Likelihood}
\StringTok{for (y in 1:(n.years{-}1))\{}
\StringTok{   r[y] \textasciitilde{} dnorm(mean.r.est, tau.proc.est)}
...
\CommentTok{\# Initial values.}
\NormalTok{jags.inits }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{}\FunctionTok{list}\NormalTok{(}\AttributeTok{sigma.proc.est =} \FunctionTok{runif}\NormalTok{(}\AttributeTok{n=}\DecValTok{1}\NormalTok{, }\AttributeTok{min=}\DecValTok{0}\NormalTok{, }\AttributeTok{max=}\DecValTok{1}\NormalTok{), }
                              ...
                              \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA}\NormalTok{,(n.years}\DecValTok{{-}1}\NormalTok{))))\}}

\NormalTok{model.file }\OtherTok{\textless{}{-}} \StringTok{\textquotesingle{}PopTrend\_ExpGr.txt\textquotesingle{}}

\CommentTok{\# Parameters monitored}
\NormalTok{jags.params }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"mean.r.est"}\NormalTok{, }\StringTok{"sigma.proc.est"}\NormalTok{, }\StringTok{"sigma.obs.est"}\NormalTok{, }\StringTok{"C.est"}\NormalTok{,}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{bookdown_files/figure-latex/ExpTrendPlot-1} \caption{Example of fitted exponential growth model for survey catch versus year.}\label{fig:ExpTrendPlot}
\end{figure}

We use the step() function\index{step function} (calculated parameter p.pos) to estimate the probability that the mean population growth rate is positive. We obtained probabilities of 0.73, 0.73, 0.40, 0.44, and 0.61 for five trials using the default settings, compared to 0.74, 0.69, 0.92, 0.99, and 0.91 for a simulation mean growth rate of 1.05. These results show the challenge in detecting a population trend, which will depend on the magnitude and annual variation in the mean annual change, the length of the time series, and the precision of the survey.
...
\NormalTok{N[}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ N1}
\NormalTok{lambda }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{((n.years}\SpecialCharTok{+}\NormalTok{p.years}\DecValTok{{-}1}\NormalTok{), mean.lambda, sigma.lambda) }
  \CommentTok{\# Draw vector of annual growth rates}
...
\StringTok{logC.est[1] \textasciitilde{} dunif(1, 100)     \# Initial relative abundance}
\StringTok{mean.r.est \textasciitilde{} dnorm(0, 0.001)    \# Mean growth rate}
\StringTok{sigma.proc.est \textasciitilde{} dunif(0, 1)  \# Annual variation in r}
\StringTok{tau.proc.est \textless{}{-} pow(sigma.proc.est, {-}2)}
\StringTok{sigma.obs.est \textasciitilde{} dunif(0, 100)     \# Observation error}
\StringTok{tau.obs.est \textless{}{-} pow(sigma.obs.est, {-}2)}
...
\StringTok{   \} \#y}
\StringTok{p.pos \textless{}{-} step(mean.r.est)}
\StringTok{p.increase \textless{}{-} step(C.est[n.years+p.years]{-}C.est[1])}
\StringTok{  \# Prob of increase from first to last}
\StringTok{\}}
\StringTok{    "}\NormalTok{,}\AttributeTok{fill =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{sink}\NormalTok{()}
...
We can now plot the observed catches, augmented by the predicted catch vector and lines representing lower and upper 95\% credible intervals\index{credible interval} \citep{kéry.schaub_2011}. The quantile() function\index{quantile function} provides the 0.025 and 0.975 quantiles from the vector of retained MCMC updates (draws from the posterior distribution). The ylim plot option allows us to scale the y-axis to accommodate all four series.
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{bookdown_files/figure-latex/ForecastTrendPlot-1} \caption{Example of fitted exponential growth model for survey catch versus year with forecasts for five additional years. Dashed lines represent lower and upper 95\% credible intervals.}\label{fig:ForecastTrendPlot}
\end{figure}
...
\StringTok{ MeanR.est \textasciitilde{} dlnorm(0, 1E{-}6) \# uninformative prior (\textgreater{}0)}
...
\CommentTok{\# Initial values}
\NormalTok{Rec.init }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{((n1}\SpecialCharTok{+}\DecValTok{1}\NormalTok{)}\SpecialCharTok{*}\NormalTok{(n2}\SpecialCharTok{+}\DecValTok{1}\NormalTok{)}\SpecialCharTok{/}\NormalTok{(m2}\SpecialCharTok{+}\DecValTok{1}\NormalTok{)}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{) }\CommentTok{\# Chapman modification }
\NormalTok{jags.inits }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{ }\FunctionTok{list}\NormalTok{(}\AttributeTok{MeanR.est=}\FunctionTok{rnorm}\NormalTok{(}\AttributeTok{n=}\DecValTok{1}\NormalTok{, }\AttributeTok{mean=}\NormalTok{Rec.init, }\AttributeTok{sd=}\DecValTok{100}\NormalTok{),}
...
\FunctionTok{hist}\NormalTok{(jagsfit}\SpecialCharTok{$}\NormalTok{BUGSoutput}\SpecialCharTok{$}\NormalTok{sims.list}\SpecialCharTok{$}\NormalTok{MeanR.est, }\AttributeTok{main=}\StringTok{""}\NormalTok{, }\AttributeTok{xlab=}\StringTok{"Mean recruitment"}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v=}\NormalTok{MeanR, }\AttributeTok{col=}\StringTok{"red"}\NormalTok{)}
...
The simulation code builds on that used in the cohort-specific catch-curve analysis (\ref{Cohort}) and recruitment (\ref{Recruit}) sections. The number of ages is arbitrary but should be sufficient to characterize the selectivity patterns for the fishery and survey. Annual fishing mortality\index{mortality, fishing} varies randomly between user-set lower and upper bounds. Natural mortality\index{mortality, natural} can be difficult to estimate internally \citep{quinn.deriso_1999}, so here it is fixed in the simulation and the fitted model \citep[e.g., using external information such as longevity:][]{lorenzen1996}.
