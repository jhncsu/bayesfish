...
The uniform prior distributions\index{prior distribution} chosen for this example were intended to be uninformative, and additional testing with large sample sizes showed no apparent bias. Other distributions could have been used and it is worthwhile to establish that the results are robust to the particular distributions chosen. For example, $L_\infty$, $k$, and Var_L are restricted to positive values so a vague lognormal prior (e.g., <code>dlnorm(0,1E-6))</code>) could be used (with initial values obtained from <code>rlnorm(n=1, meanlog=0, sdlog=0.5)</code>). A vague normal distribution (e.g., <code>dnorm(0,1E-6))</code>, with initial values from <code>rnorm(n=1, mean=0, sd=1)</code>) can be used for $t_0$, which can take on positive or negative values. Note that the model structure can affect whether a prior distribution is uninformative. For example, the uniform prior distribution for Var_L will not be flat when transformed to calculate precision (tau). In general, a prior distribution chosen to be uninformative will not be for transformed parameters; for example, a flat prior for standard deviation would be informative for variance [@ross_2022].
