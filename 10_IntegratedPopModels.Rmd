...
par(mfrow = c(1, 3))
plot(seq(1:(Y-1)), 
     TwoAge_jags$BUGSoutput$mean$AnnualS.est, ylab="Annual Survival", xlab="Year",
...
The simulation code builds on that used in the cohort-specific catch-curve analysis\index{catch curve analysis} (\@ref(Cohort)) and recruitment\index{recruitment} (\@ref(Recruit)) sections. The number of ages is arbitrary but should be sufficient to characterize the selectivity patterns for the fishery and survey.  Annual instantaneous fishing mortality rate\index{instantaneous fishing mortality rate} varies randomly between user-set lower and upper bounds. The instantaneous natural mortality rate\index{instantaneous natural mortality rate} can be difficult to estimate internally [@quinn.deriso_1999; @schaub.etal_2024], so here it is fixed in the simulation and the fitted model [e.g., using external information such as longevity: @lorenzen1996; @then.etal_2015].
...
par(mfrow = c(1, 2))
plot(seq(1:Y), jagsfit$BUGSoutput$mean$AnnualF.est, ylab="Annual F", 
     xlab="Year", type="b", pch=19, 
     ylim=range(AnnualF,jagsfit$BUGSoutput$mean$AnnualF.est))
...
plot(seq(1:Y), jagsfit$BUGSoutput$mean$N.est.total, ylab="Total N", 
     xlab="Year", type="b", pch=19, 
     ylim=range(N.total,jagsfit$BUGSoutput$mean$N.est.total))

points(N.total[], col="blue", type="b")

plot(seq(1:A), jagsfit$BUGSoutput$mean$SelF.est, 
     ylab="Fishery selectivity", xlab="Age", type="b", pch=19, 
     ylim=c(0,1))
points(SelF, col="blue", type="b")

plot(seq(1:A), jagsfit$BUGSoutput$mean$SelS.est,
...
1.  For the two-age model (Section \@ref(TwoAge)), compare true and estimated total abundance using absolute relative errors
